...
insert into storage.buckets (id, name, public)
values ('student-passports', 'student-passports', true) on conflict (id) do nothing;
...
-- ---------- student passports (public apply uploads) ------------------------
drop policy if exists "student_passports_read" on storage.objects;
create policy "student_passports_read" on storage.objects for select
  using (bucket_id = 'student-passports');

drop policy if exists "student_passports_write" on storage.objects;
create policy "student_passports_write" on storage.objects for insert
